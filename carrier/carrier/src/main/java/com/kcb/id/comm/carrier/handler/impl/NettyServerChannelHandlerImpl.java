...
public class NettyServerChannelHandlerImpl implements Handler{
	...
	public void onReceived(ChannelHandlerContext ctx, Object msg, Map<String,MessageInfo> messageRepository, HandlerInfo handler) {
		...
		try {
			...
			if(messageInfo.getForward()!=null && !"".equals(messageInfo.getForward()) && messageRepository.get(messageInfo.getForward()) != null) {
	...
	private void forward(ChannelHandlerContext ctx, Map<String, MessageInfo> messageRepository, MessageInfo messageInfo, MessageInfo parsedMsg) {
		...
		buf.duplicate().readBytes(forwardBytes);
		try {
	...
	private void sendMessage(ChannelHandlerContext ctx, Message msg, Map<String, Object> responseMap) {
		...
		try {
			ByteBuf b = NettyUtils.getMessage2ByteBuf(msg, responseMap);
			byte[] buf = new byte[b.readableBytes()];
			b.duplicate().readBytes(buf);
			logger.debug("response message is {}" ,new String(buf));
			ctx.write(b);
		} catch (Exception e) {
			logger.error(e.toString(),e);
		}
		...
		try {
			...
			for (int i = 0; i < repeatCount; i++) {
				for (int j = 0; j < body.length; j++) {
					...
					msg.getResponseMessage().setBodyValue(f.getName(), value, i);
				}
			}
			currentPart = "tail";
	...
	/*
	 * source 의 전문 데이터를 target의 전문데이터로 만들어
	 * 바이트 배열로 리던해 주는 메서드 
	 */
